...
class Ant:
    ...
    def get_spiders(self, scene):
        """
        Фукнция возвращает всех пауков из сцены
        """
        spiders = []
        for agent in scene:
            if agent.name == 'Spider':
                spiders.append(agent)
        return spiders

    def get_ants(self, scene):
        """
        Возвращает всех муравьев из сцены
        """
        ants = []
        for agent in scene:
            if agent.name == 'Ant':
    ...
    def get_apples(self, scene):
        """
        Возвращает все яблоки из сцены
        """
        apples = []
        for agent in scene:
            if agent.name == 'Apple':
    ...
    def get_anthill(self, scene):
        """
        Возвращает муравейники сцены(задел на будущее)
        """
        anthill_1 = None
        for agent in scene:
            if agent.name == 'Anthill':
                anthill_1 = agent
        return anthill_1

    def body(self):
        """
        Построение тела на карте
        """
    ...
    def self_determination(self, state, ants, spiders, apples):
        """
        Самоопределение.
        ...
        if state[0] == 4:  # Если параметр равен 4, то убегать от паука(см. комментрий к переменной state)
            ...
            if state[1] not in spiders:  # если преследующего паука больше нет в радиусе, то отменить бегство и начать
        ...
        if state[0] == 0:  # Если состояние - простой поиск яблока
            if len(ants) < 5 and spiders != []:  # Если нет муравьев и есть пауки - бегство
                ...
                if self.get_distance(apples[0]) <= 5:
                    return [1, apples[0]]
        return state

    def live_by_self_determination(self, geo, state, ants, spiders, apples):
        """
        Жить по самоопределению
        """
        if geo[0] >= 990 or geo[0] <= 10 or geo[1] >= 990 or geo[1] <= 10:

            if geo[0] >= 990:
                self.u = math.pi + self.u
            elif geo[0] <= 10:
                self.u = math.pi + self.u
            if geo[1] >= 990:
                self.usin = math.pi + self.usin
            elif geo[1] <= 10:
                self.usin = math.pi + self.usin
        else:
            if state[0] == 0:  # в поиске яблока
                if spiders != []:
                    consensus = self.send_message_to_radius(0, ants, spiders[0])
                    if consensus == True:
                        self.state = [2, spiders[0]]
            ...
            elif state[0] == 1:  # нашёл яблоко и тащит его
            ...
            elif state[0] == 3:  # конец игры (возвращение в муравейник)
                self.u = math.acos((self.anthill.geo[0] - geo[0]) / self.get_distance(self.anthill))
                self.usin = math.asin((self.anthill.geo[1] - geo[1]) / self.get_distance(self.anthill))
                if self.get_distance(self.anthill) <= 4:
                    self.die(self)

            elif state[0] == 4:  # бегство от угрозы
    ...
    def move(self, scene):
        ...
        for agent in self.scene:
            full_scene.append(agent)  # после окончания хода, паук передает в сцену изменившиеся данные и возвращает ее
            # диспетчеру вместе с ходом(простите, без элементарного диспетчера не получался нормальный паук)
        return full_scene

    def get_distance(self, obj):
        """
        Возвращает информацию о расстоянии до объекта при помощи любимой теоремы Пифагора
        """
        return math.sqrt((self.geo[0] - obj.geo[0]) ** 2 + (self.geo[1] - obj.geo[1]) ** 2)

    def accept_message(self, param, obj):
        """
        Функция принятия сообщения. 0 - принятия просьбы о помощи
        """
        if param == 0:
    ...
    def send_message_to_radius(self, param, ants, obj=0):
        """
        Функция разослать сообщение всем в радиусе. param = 0 - помогите убить паука
        """
        if param == 0:
            if len(ants) > 3:
                for ant in ants:
    ...
    def run(self):
        """
        Метод, который перемещает муравья в нужном направлении,
        после расчета хода(сделан отдельно, т. к. в будущем можно будет отделить планировщик от рендеринга)
        """
