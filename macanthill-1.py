...
class Ant:
    ...
    def closest_apple(self):
        dl = sorted(
            [[((abs(round(apl.geo[0] - self.geo[0]) ** 2 + (apl.geo[1] - self.geo[1]) ** 2) ** 0.5)), apl] for apl in
             apples], key=lambda x: x[0])
        if dl:
            return dl[0][1]
    ...
    def run_away_from_spiders(self):
        distance = [[((spider.geo[0] - self.geo[0]) ** 2 + (spider.geo[1] - self.geo[1]) ** 2) ** 0.5, spider] for
                    spider in spiders]
        for el in distance:
            if el[0] < 100:
                if not self.isready:
...
class Spider:
    ...
    def closest_ant(self):
        dl = sorted(
            [[((abs(round(ant.geo[0] - self.geo[0]) ** 2 + (ant.geo[1] - self.geo[1]) ** 2) ** 0.5)), ant] for ant in
             ants if not ant.isready], key=lambda x: x[0])
