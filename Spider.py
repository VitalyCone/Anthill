...
class Spider:
    def __init__(self, scene):
        ...
        self.array_of_key = [-0.2,                   0.3,                 0.2,  0.6] #коэфициенты потребностей(переписать их, чтобы сумма была равна единице)


    def body(self):
        return pygame.Rect(self.geo[0],self.geo[1],15,15)
    
    def get_num_of_spiders_around(self, geo):   #метод, который обрабатывает сцену, и ищет в ней количество пауков, в радиусе от заданных координат
        num_of_spiders = 0
        for spider in self.scene:
            if spider.name == Spider and (abs(geo[0] - spider.geo[0]) <= self.r) and (abs(geo[0] - spider.geo[0]) <= self.r) and spider.geo == self.geo:
                num_of_spiders += 1
    ...
    def get_need(self, u):
        ...
        if num_of_spiders != 0:
            needs.append(1) #рассчет коэфициента для выбора направления из количество пауков. Есть другие пауки - 0, нет - 1
        else:
            needs.append(0)
        if num_of_ants > 5:
        ...
        for i in range(0, len(self.array_of_key)):
            sum_of_needs += needs[i]*self.array_of_key[i]  #рассчет удовлетворенности паука, учитывая весовые коэфициенты каждого параметра.
    ...
    def move(self, scene): #метод для рассчета действий для хода муравья
        ...
        if len(ants) == 0: # если вокруг паука нет муравьев, то он продолжает находиться в состоянии поиска
                ...
                while a < math.pi*2: #вычисляем лучшие углы поворота, при помощи метода get_need()
                    ...
                    if move1[0] > best_move[0]:
                        ...
                        best_moves.append(move1)
                choice = random.choice(best_moves) #если лучших ходов несколько, то выбираем случайный(чем больше параметров, тем меньше вероятность выбора случайного направления)
                self.u = choice[2]  
            ...
            for ant in ants:        #каждый ход охоты идет проверка, точно ли выбранный муравей - лучший.
                if self.get_energy(ant) > self.get_energy(best_ant): # если полученная энергия больше полученной энергии при охоте за лучшим муравьем, тогда назначается новый лучший муравей 
                    best_ant = ant
            ...
            if distance < (self.speed+self.my_ant.speed) and self.my_ant != None: #если же муравей оказался на дистанции меньшей, чем минимальное перемещение за ход, тогда муравей умирает
        ...
        for agent in self.scene:
            full_scene.append(agent) #после окончания хода, паук передает в сцену изменившиеся данные и возвращает ее диспетчеру вместе с ходом(простите, без элементарного диспетчера не получался нормальный паук)
        return full_scene
    ...
    def die(self):
        if self.energy < 0: #тут правда что-то будет, но сначала надо определиться с рождением новых агентов и прописать нелинейную зависимость(см. ссылки на википедию)
